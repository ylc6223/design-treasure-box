  ...
  describe('Resources Data', () => {
    ...
    it('should cover all categories', () => {
      ...
      categoryIds.forEach((categoryId) => {
        expect(usedCategoryIds.has(categoryId), `Category ${categoryId} has no resources`).toBe(
          true
        );
      });
    });
  ...
  describe('Data Integrity', () => {
    it('should have resources distributed across all categories', () => {
      ...
      resourcesByCategory.forEach(({ categoryId, count }) => {
        expect(count, `Category ${categoryId} should have at least one resource`).toBeGreaterThan(
          0
        );
      });
    });
