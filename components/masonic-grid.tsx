...
const MasonryCard = React.memo<MasonryCardProps>(
  ({ data, width: _width, isFavorited, onFavorite }) => {
    return <ResourceCard resource={data} isFavorited={isFavorited} onFavorite={onFavorite} />;
  }
);
...
export function MasonicGrid({
    ...
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <div className="text-center">
          <p className="text-lg font-medium text-[var(--text-secondary)]">暂无资源</p>
          <p className="mt-2 text-sm text-[var(--text-muted)]">请尝试其他筛选条件</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      <Masonry
        key={`${resources.length}-${resources[0]?.id || 'empty'}`}
        items={resources}
        columnWidth={280}
        columnGutter={24}
        rowGutter={24}
      ...
      {!hasMore && resources.length > 0 && (
        <div className="mt-12 text-center">
          <p className="text-sm text-[var(--text-muted)]">已加载全部 {resources.length} 个资源</p>
        </div>
      )}
    </div>
  );
