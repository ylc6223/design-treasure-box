    ...
    <div className={cn('space-y-3', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            {!isUserRating && <span className="text-xs text-text-muted">策展人评分</span>}
          </div>
        </div>

        {/* 评分按钮 */}
        {isAuthenticated && onRate && (
          <Button variant="outline" size="sm" onClick={onRate} className="gap-2">
      ...
      {userRating && (
        <div className="rounded-lg border border-border bg-surface/50 p-3 space-y-2">
          ...
          {userRating.comment && (
            <p className="text-sm text-text-secondary line-clamp-2">{userRating.comment}</p>
          )}
        </div>
      )}

      {/* 未登录提示 */}
      {!isAuthenticated && <p className="text-xs text-text-muted">登录后可以评分</p>}
    </div>
  );
}
